    ...
    <div className="bg-slate-50/80 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800 backdrop-blur-md relative md:sticky md:top-[61px] z-[40]">
      <div className="max-w-[1600px] mx-auto px-4 py-2 flex items-center gap-3 md:gap-4 overflow-x-auto no-scrollbar">
        ...
        <div className="flex items-center gap-2 ml-auto shrink-0">
          {mode === 'compare' && (
            <>
              <button onClick={() => setShowManagePlayers(true)} className="px-4 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-xs font-black text-slate-500 uppercase hover:text-slate-900 dark:hover:text-white transition-all">Players</button>
              <button onClick={() => setShowAll(!showAll)} className={`px-4 py-1.5 rounded-lg text-xs font-black uppercase transition-all border ${showAll ? 'bg-sky-600 border-sky-600 text-white' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-500'}`}>
          ...
          {section === 'standings' && (
            <button onClick={() => whatIfEnabled ? setWhatIfEnabled(false) : setShowWhatIfConfirm(true)} className={`flex items-center gap-2 px-4 py-1.5 rounded-lg text-xs font-black uppercase transition-all border ${whatIfEnabled ? 'bg-amber-500 border-amber-500 text-white' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-500'}`}>
              <TrendingUp className="w-4 h-4" /> <span className="hidden sm:inline">What-If</span>
            </button>
          )}
        </div>
      </div>
