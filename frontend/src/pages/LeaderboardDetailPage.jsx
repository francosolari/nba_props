...
import { ChevronLeft, Grid, Expand, Minimize2, X, Search, AlertTriangle, Pin, PinOff } from 'lucide-react';
                          ...
                          <Draggable key={`W-${row.team}`} draggableId={`W-${row.team}`} index={index} isDragDisabled={!whatIfEnabled}>
                            {(prov)=> {
                              ...
                              <tr ref={prov.innerRef} {...prov.draggableProps} {...prov.dragHandleProps} className={isChanged ? 'bg-amber-50' : ''}>
                                <td className="sticky left-0 z-10 bg-rose-50/50 backdrop-blur px-3 py-2 text-sm font-medium text-slate-800 border-b border-slate-100" style={{ minWidth: teamColWidth, width: teamColWidth }}>
                                  <div className="flex items-center gap-2">
                                ...
                                {displayedUsers.map((e) => {
                                  ...
                                  return (
                                    <td key={`c-${e.user.id}-${row.team}`} className="px-3 py-2 border-b border-slate-100">
                                      <span className={`inline-flex items-center justify-center w-full px-2 py-1 rounded-md border text-xs ${color}`} title={`Pred: ${predPos} • Points: ${pts}`}>
                                        {predPos}
                                      </span>
                                    </td>
                                  );
                        ...
                        {(eastOrder.length? eastOrder.map((it,idx)=>({team: it.team, conference:'East', actual_position: standingsTeams.find(r=>r.team===it.team)?.actual_position })) : standingsTeams.filter(r=> (r.conference||'').toLowerCase().startsWith('e'))).map((row, index)=> (
                          <Draggable key={`E-${row.team}`} draggableId={`E-${row.team}`} index={index} isDragDisabled={!whatIfEnabled}>
                            {(prov)=> {
                              ...
                              return (
                              <tr ref={prov.innerRef} {...prov.draggableProps} {...prov.dragHandleProps} className={isChanged ? 'bg-amber-50' : ''}>
                                <td className="sticky left-0 z-10 bg-sky-50/50 backdrop-blur px-3 py-2 text-sm font-medium text-slate-800 border-b border-slate-100" style={{ minWidth: teamColWidth, width: teamColWidth }}>
                                  <div className="flex items-center gap-2">
                    ...
                    return qArr.map(q => (
                      <tr key={`q-${q.id}`} className="odd:bg-white/70 even:bg-white/40">
                        ...
                        <td key={`q-${q.id}-${e.user.id}`} className="px-3 py-2 border-b border-slate-100">
                          <span className={`inline-flex items-center w-full truncate px-2 py-1 rounded-md border text-xs ${color}`} title={`${answer} • +${pts}`}>
                            {answer || '—'}{pts ? `  +${pts}` : ''}
                          </span>
                        </td>
                          );
