...
import type {
  ...
  StandingsTeam,
  WhatIfItem,
} from './LeaderboardDetailPage.types';


function LeaderboardDetailPage({ seasonSlug: initialSeasonSlug = 'current' }: LeaderboardDetailPageProps) {
  const paramsRoot = document.getElementById('leaderboard-detail-root');
  const initialSection = (paramsRoot?.getAttribute('data-initial-section') || 'standings') as SectionType;
  ...
  const [selectedSeason, setSelectedSeason] = useState<string>(initialSeasonSlug);

  // Fetch seasons where the user has participated (has submissions)
  const { data: seasonsData } = useQuery<SeasonData[]>({
  ...
  const [mode, setMode] = useState<ModeType>('compare');
  ...
  const [sortBy, setSortBy] = useState<SortByType>('standings');
  const [query, setQuery] = useState<string>('');
  const [whatIfEnabled, setWhatIfEnabled] = useState<boolean>(false);
  const [showWhatIfConfirm, setShowWhatIfConfirm] = useState<boolean>(false);
  const [pinnedUserIds, setPinnedUserIds] = useState<string[]>([]);
  const [collapsedWest, setCollapsedWest] = useState<boolean>(false);
  const [collapsedEast, setCollapsedEast] = useState<boolean>(false);
  ...
  const [mobileStickyOffset, setMobileStickyOffset] = useState<number>(0);
  const [showMobileDragTooltip, setShowMobileDragTooltip] = useState<boolean>(() => {
    try {
  ...
  const primaryUserId = useMemo<string>(() => {
  ...
  const loggedInEntry = useMemo<LeaderboardEntry | undefined>(() => {
  ...
  const standingsTeams = useMemo<StandingsTeam[]>(() => {
    ...
    const orderKey = (r: StandingsTeam) => [r.conference === 'West' ? 0 : 1, r.actual_position || 999, r.team];
    list.sort((a, b) => {
  ...
  const [westOrder, setWestOrder] = useState<WhatIfItem[]>([]);
  const [eastOrder, setEastOrder] = useState<WhatIfItem[]>([]);
...
useEffect(() => {
  ...
  const standingPoints = (pred: number | null, actual: number | null): number => {
  ...
  const onDragEnd = (result: DropResult): void => {
    ...
    const reorder = (list: WhatIfItem[], startIndex: number, endIndex: number): WhatIfItem[] => {
  ...
  const addUser = (id: string | number): void => {
    if (!id) return;
    setSelectedUserIds(prev => Array.from(new Set([...prev, String(id)])));
  };

  const togglePin = (id: string | number): void => {
    if (!id) return;
  ...
  const dismissMobileDragTooltip = (): void => {
  ...
  const didReadUrlRef = useRef<boolean>(false);
  useEffect(() => {
  ...
  // ────────────────────────── Mobile UI ──────────────────────────
  const fromSectionKey = (s: SectionType): string => s === 'standings' ? 'Regular Season Standings' : s === 'awards' ? 'Player Awards' : 'Props & Yes/No';
